...
\section{Discussion}
To summarize the results we can say, that the combination between generators and evaluators plays a huge role in the results. In case of the text generators the difference seems minor, but for example the roberta base evaluator detected around ten falcon generated texts as human made in the middle case of fig. \ref{fig:falcon} before and after process, which means that if that would not have been the case, we would have a perfect score here.
