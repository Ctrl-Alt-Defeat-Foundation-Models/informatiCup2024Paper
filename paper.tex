...
\section{Conclusion}
Overall we did not find any general fooling technique that can be considered in new AI detectors. We discovered that the detector both for the image and text is highly dependent on the used generator and therefore a processor might reduce the detection rate for some detectors, but on others increase the rate. Our pipeline consisting of creation, process and detection is simple but effective and can be used in further research.
