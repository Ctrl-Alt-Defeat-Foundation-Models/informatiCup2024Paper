...
A similar model, but in a more recent version is the GPT-4 model. The model is yet to be released by it's developer OpenAI, but is available through an API. Compared to its predecessors GPT-4 is a multimodal model, which is not only capable of using text for input but also images. The model is trained on Next-Token-Prediction and is capable and was post-training aligned to better adhere to desired behaviour. The model is capable to perform well in different professional benchmarks out of a variety of domains and can demonstrate it's performance in different languages. \cite{openai2023gpt4}
...
The \textbf{Llama 2} model built and released by Meta. It is available in different sizes, ranging from 7 billion to 70 billion parameters and different appliances like LLAMA 2-Chat, which is optimized for dialogue interaction. When it was released in 2023, it outperformed state-of-the-art open source models. A main refinement compared to its predecessor LLAMA, was the focus on transparency and safety including a post-training alignment. \cite{touvron2023llama}

Another generative model for text is \textbf{BLOOM}. This huge 176 billion parameter big model was published in 2023 and is openly available. The model is multilingual and was built in a decentralized manner by many different contributors. Hundreds of researchers helped with the development of the architecture and tokenizer. The final model was trained on the Jean Zay supercomputer. It is built with a decoder-only architecture and used the ROOTS corpus dataset for training. \cite{workshop2023bloom}
